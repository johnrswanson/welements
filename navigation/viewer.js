		...
		$.getJSON(url,function(json){
			$.each(json.boxiteminfo,function(i,bdat){
				...
				'');
					...
					if(bdat.photo!=''){
				$('#boxelement_'+bdat.ID).append(''+
				'<div class="boxphotowrap"><a href="#" onclick="loadContent('+bdat.ID+');"><img class="boxphoto" src="img/full/'+bdat.photo+'" ></a></div>');
				}
...
'<input type="radio" name="publish" value="n"> No'+
	...
	$('#lightbox>#content').html('<div class="boxtitle">Edit Box Item</div>');
		...
		$.getJSON(url,function(json){
			$.each(json.boxiteminfo,function(i,idat){
				...
				$("#lightbox>#content").append(''+
					...
					'<form  ID="formElement">'+
					'<input type="hidden" name="editboxitem" value="'+idat.ID+'">'+
		...
		'<input type="text" name="title" value="'+idat.title+'">'+
			'<textarea style="width: 100%; max-width: 100% ; min-height: 100px; margin: auto;" name="pagecontent"> '+safetext+'</textarea>'+
			...
			'<input style="  " ID="saveedit" type="button" name="submit" '+
						'value="Save" onclick="editNow('+idat.boxID+'); ">'+
